  ...
  <db-content v-slot="slotProps">
    <div class="font-inter" v-if="slotProps.userStore">
      ...
      <header class="w-full flex justify-between items-center mb-6 mt-4">
        <corep-logo-notext :size="40" :bg-color="slotProps.mode ? '#3B3F4D' : '#EEEEEE'"
          :text-color="slotProps.mode ? '#fff' : '#212939'" class="md:hidden mx-2"></corep-logo-notext>
        <div class="w-full max-w-lg md:max-w-auto hidden md:block">
          <v-input class="dark:bg-db-pry-dark min-w-[70%] block py-2 px-3 dark:text-white rounded-lg" size="x-small"
            style-type="search" :icon="SearchIcon" placeholder="Search for Courses" full iconLeft
            :icon-color="slotProps.mode ? '#ffff' : '#212939'"></v-input>
        </div>
        <div class="flex items-center mx-2">
          <div class="w-10 h-10 flex justify-center items-center hover:bg-sec rounded-full mx-4">
            <router-link to="/notifications">
              <div class="flex relative">
                <bell-icon class="" :text-color="slotProps.mode ? '#ffff' : '#212939'"></bell-icon>
                <div class="w-2 h-2 bg-red-400 top-1 left-1 absolute rounded-full"></div>
              </div>
            </router-link>
          </div>
          <router-link to="/settings/user">
            <div class="flex items-center font-inter text-sm">
              <p class="hidden md:block dark:text-white text-db-pry-dark mr-2" v-if="slotProps.userStore">
                {{ slotProps.userStore.username }}
              </p>
              <div class="flex bg-pry rounded-full w-10 h-10 justify-center items-center">
                <img src="/m-header.png" alt="avataar" class="rounded-full p-[4px]" />
              </div>
            </div>
          </router-link>
        </div>
      ...
      <div class="main-content-index py-6 md:py-auto mx-2 md:mx-auto pt-10">
        <div class="app-todos mt-6 md:mt-0" v-if="slotProps.userStore.user_todo_level !== 'completed'">
        ...
        <div class="main mt-6 md:mt-0" v-else>
          <p>User has completed all his todos</p>
          <v-button type="sec" @click="saveMessagesDeviceToken">Request Permission for Notification</v-button>
        </div>
      </div>
    </div>
  </db-content>
...
const { saveMessagesDeviceToken } = useFirebaseNotification()
definePageMeta({
  layout: "d-board",
