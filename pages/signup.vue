  ...
  <div class="flex min-h-screen">
    <div class="flex items-center w-full max-w-3xl p-8 mx-auto lg:px-12 lg:w-6/12">
      <div class="bg-white rounded-xl max-w-md mx-auto font-inter w-full">
        ...
        <form @submit.prevent="submitHandler" class="pt-4">
          <stack class="mx-auto" :gap="3">
            <h2 class="font-bold text-2xl text-left">Create Account</h2>
            <v-input
              placeholder="Classydev"
            ...
            <v-drop-down
              ...
              v-if="schools"
              @chosen="setSchool"
              :error="formReactive.id.error"
            >
            </v-drop-down>
            <v-input
              type="text"
              placeholder="Enter School Email addresss"
              full
              styleType="white"
              class="text-sm rounded-lg"
              size="small"
            ...
            <k-dropdown
              ...
              :label="levelDropDownLabel"
              placeholder="Search for school"
              :drop-state="dropState2"
              :error="formReactive.level.error"
              @on-search="searchDropDown"
              @on-dropdown-click="handleDropdown"
              @on-close-dropdown="closeDropDown"
            >
              <ul class="" v-if="levels">
                <li
                  class="block w-full text-sm cursor-pointer hover:bg-[#eee] rounded-md mt-2 p-2"
                  @click="chooseFromDropDown($event, item.id)"
                ...
                >
                  ({{ item.name }} level) - {{ item.year }} year
                </li>
              </ul>
              <p v-else>Loading ...</p>
            </k-dropdown>
            ...
            <h3 class="text-sm text-center font-bold">
              By signing up, you agree to our
              <router-link to="#"
                ><span class="text-pry">Terms and Conditions</span></router-link
              >
            </h3>
          </stack>
        </form>
        <div class="text-center py-5 md:py-8 font-ibmplex">
          <p>
            Already have account?<span
              class="opacity-70 underline-none ml-1 text-pry-dark"
            >
...
async function requestSchools() {
  try {
    let req = new Graph()
      .service("School/getAll")
      .selectAll("id", "short_name", "full_name");
