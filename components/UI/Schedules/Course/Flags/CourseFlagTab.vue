<template>
    <div class=" w-full ">
        <h2 class="dark:text-white text-lg font-semibold md:text-xl my-4 ml-10">Class Flags</h2>
        <div class="max-w-[80%] ml-10 mt-6 min-h-[300px] flex flex-col justify-center">
            ...
            <loader-icon class=" w-full text-center mx-auto" :size="50" :color="darkMode ? '#fff' : '#212939'"
                v-else-if="flags.length === 0 && loadingFlags"></loader-icon>
        ...
        <div
            class="sticky bottom-[3.5rem] py-6  md:bottom-0 flex w-full flex-1 dark:bg-db-pry bg-db-white-dark dark-grad md:py-4 max-h-[200px]">
            <form class="relative flex justify-center w-full md:w-[80%] mx-auto px-4 md:px-0"
                ...
                <v-button type="sec" class="my-auto absolute bottom-1 right-6 md:right-1  px-2" size="x-small" :loading="loadingCreateNewMessage"><send-icon
                        class="w-5" /></v-button>
            </form>
            <!-- <form
...
import { useFlags, loadingFlags } from '~~/composables/classes/flags/useFlags';
import { useUserStore } from '~~/store/user';

const { createFlagMessage, getAllFlags, getCurrentViewingParamId, flags, watchCourseFlags, loadingCreateNewMessage } = useFlags()
...
<style scoped>
.dark-grad {
    background: linear-gradient(180deg, rgba(33, 41, 57, 0), #3B3F4D 58.85%);
}
</style>