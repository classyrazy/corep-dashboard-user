...
export const useFirebaseNotification = () => {
  ...
  async function requestPermission() {
    try {
      ...
      if (permission === "granted") {
        ...
        useAlert().openAlert({
          type: "Alert",
          msg: "Accept Permission to get notified about your courses",
        });
      }
    } catch (error) {
      console.log(error);
  ...
  async function saveMessagesDeviceToken() {
    let fcmToken = await getToken(messaging, {
      vapidKey: config.VITE_FIREBASE_VAPID_KEY as string,
    });
    console.log("fcmToken", fcmToken);
    useAlert().openAlert({
      type: "Alert",
      msg: "new foreground notification after fcmtoken has been gotten",
    });

    if (fcmToken) {
      try {
        let req = await new Graph()
          .service("CourseSubscription/notificationPermissionAccepted")
        ...
        onMessage(messaging, async (payload) => {
          ...
          // if (payload) {
          await new Notification(payload.notification?.title || "new notification", {
